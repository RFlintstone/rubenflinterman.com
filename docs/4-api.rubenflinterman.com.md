...
1. **Prepare Secrets**: Ensure all files in the `./secrets` folder exist by running 'secret_gen.sh'.
2. **Hoop.dev**: Install and configure [hoop.dev](https://hoop.dev/docs/getting-started/cli) according to the documentation.
   - Ensure the `hoop.dev` environment is active to securely manage the production database.
3. **Docker Compose**: Configure docker-compose Run `docker-compose up -d`.
...
4. **Kubernetes Deployment**: Create and configure your deployment by using:
   - An ingress for inbound traffic
   - A service to forward te traffic to your deployment
   - The deployment to start and serve the API

---

### Database Migrations
Database migrations can be executed using 'kubectl port-forward' as well as 'hoop.dev'. You should do this when you are updating the data model (e.g., adding the `Username` field):
1. **Generate**: `dotnet ef migrations add MigrationName`
2. **Push**: `dotnet ef database update`
   - *Docker Deployment: Ensure your local environment can reach the DB at `localhost:5433` as defined in the docker-compose ports.*
   - *Kubernetes Deployment: Ensure your local environment can reach the remote DB by setting up a tunnel between your cluster and the device you plan to execute the migration with.*
