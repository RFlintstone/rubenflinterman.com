...
export default function AppLayout({
  ...
  return (
    <div className="min-h-screen bg-[#0a0c10] text-slate-200 font-sans">
      <div className="flex">
        ...
        <aside className="hidden md:flex flex-col w-72 h-screen sticky top-0 bg-[#0d1117] border-r border-slate-800 p-6">
            ...
            {isCampaignMenuOpen && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-20 overflow-hidden">
                {campaigns.map(c => (
                  <button 
                    key={c.id}
                    onClick={() => { setActiveCampaign(c); setIsCampaignMenuOpen(false); }}
          ...
          <nav className="flex-1 space-y-2">
            ...
            <SidebarItem icon={Quote} label="Quotes" active={pathname === '/quotes'} onClick={() => router.push('/quotes')} color={themeColor} />
            <SidebarItem icon={Book} label="Lore" active={pathname === '/lore'} onClick={() => router.push('/lore')} color={themeColor} />
            <SidebarItem icon={Users} label="The Party" active={pathname === '/party'} onClick={() => router.push('/party')} color={themeColor} />
            <SidebarItem icon={MapIcon} label="Map" active={pathname === '/map'} onClick={() => router.push('/map')} color={themeColor} />
          </nav>
      ...
      <div className={`fixed top-0 right-0 w-[500px] h-[500px] ${activeCampaign.theme === 'blue' ? 'bg-blue-900/10' : 'bg-amber-900/10'} rounded-full blur-[120px] pointer-events-none -z-10`}></div>
    </div>
  );
}
